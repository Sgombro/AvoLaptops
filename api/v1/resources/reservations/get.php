...
$query = "SELECT r.id_reservation, r.date, r.time_start, r.time_end, r.status, r.class, r.classroom, u.id_user, u.name, u.surname, u.email, u.password, u.role, laptops.id_laptop, m.*, l.* from reservations r
...
function qualify_column($column){
    $ambiguous_keys = ["id_user", "id_laptop", "id_model", "id_locker", "status", "class", "classroom"];
    if(in_array($column, $ambiguous_keys)){
        ...
        if($column === "class") return "r.class";
        if($column === "classroom") return "r.classroom";
    }
    return $column;
}
...
if($params_count)
    $params = " WHERE " . $params;
$query = "SELECT r.id_reservation, r.date, r.time_start, r.time_end, r.status, r.class, r.classroom, u.id_user, u.name, u.surname, u.email, u.role, laptops.id_laptop, m.*, l.* from reservations r
INNER JOIN laptops ON laptops.id_laptop = r.id_laptop
INNER JOIN models m ON laptops.id_model = m.id_model 
INNER JOIN lockers l ON laptops.id_locker = l.id_locker
INNER JOIN users u ON u.id_user = r.id_user";
...
$payload["reservations"] = [];
    foreach($rows as $row){
        ...
        foreach($row as $attribute => $element){
            ...
            else if($counter >= 14 and $counter <= 19){
                $laptop["model"] += [$attribute => $element];
            }
            else if($counter >= 20 and $counter <= 23){
                $laptop["locker"] += [$attribute => $element];
            }
            else if($counter >= 7 and $counter <= 11){
